...
(() => {
  ...
  function fmtDate(value){if(!value)return'Time unavailable';const d=new Date(value);if(Number.isNaN(d.getTime()))return String(value);return `${d.toLocaleString('en-US',{...ET,weekday:'short',month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})} ET`;}
  function dayKey(value){const d=new Date(value);if(Number.isNaN(d.getTime()))return'Upcoming';return d.toLocaleDateString('en-US',{...ET,weekday:'long',month:'long',day:'numeric'});}
  function timeOnly(value){const d=new Date(value);if(Number.isNaN(d.getTime()))return'—';return `${d.toLocaleTimeString('en-US',{...ET,hour:'numeric',minute:'2-digit'})} ET`;}
  ...
  function buttonHtml(){const e=selected();return `<button class="pbe19-event-button" id="pbe19-event-button" type="button"><span class="pbe19-event-dot"></span><span class="pbe19-event-copy"><span class="pbe19-event-title">${esc(e?`${e.away} @ ${e.home}`:'Selected NFL Event')}</span><span class="pbe19-event-sub">${esc(e?.start?fmtDate(e.start):(e?'Time to be announced':'Loading the slate'))}</span></span><span class="pbe19-event-chevron">⌄</span></button>`;}
