 ...
 *
 * Access V2: Injuries is a public route and there is no site-wide wall, so the
 * page is tested as an anonymous reader against live production APIs. The shell
 * contract (scripts/nfl-access-contract.mjs) fails the run if the workspace is
 * walled off again. The layout assertions are unchanged.
 */
...
let shell=null;for(let i=0;i<20;i++){shell=await probe(SHELL_PROBE);if(!judgeShell(shell))break;await sleep(500)}
out(`access shell ${JSON.stringify(shell)}`);
const accessIssue=judgeShell(shell,{expect:'anonymous'});
...
let desktop=await probe(`(()=>{
  ...
  return{
    ...
    cards:cards.length,
    stories:Number((/(\\d+) current injury stor/.exec(root.querySelector('.pbe13-now-line')?.textContent||'')||[])[1]??NaN),
    emptyCoverage:!!root.querySelector('.pbe13-editorial-empty'),
...
/* availabilityRows >= 1, not >= 3: how many players the live feed reports
   hurt is news volume, not layout. On 2026-09-13 23:40Z the feed carried two
   and main went red for it. The board must render at least one row, and every
   structural and readability assertion below still applies to each. */
/* Coverage volume is live news, not layout (as 2fa06a8 did for availability
   rows): a fixed "at least five stories" failed whenever the feed carried
   four. The invariant is that the page accounts for every story it reports:
   the lead plus one coverage row for each remaining story, or the explicit
   empty state when the lead is the only one. */
if(!desktop||typeof desktop!=='object'||desktop.root!==true||desktop.heroHeight>255||desktop.lead!==true||desktop.leadImage!==true||desktop.leadMediaWidth<40||desktop.leadMediaWidth>220||desktop.maxMediaHeight>220||!(desktop.stories>=1&&(desktop.cards===desktop.stories-1)&&(desktop.cards>0||desktop.emptyCoverage===true))||desktop.articleLinks<6||desktop.badLinks!==0||desktop.images<1||desktop.upscaled!==0||desktop.loadedImages<1||desktop.broken>0||desktop.telemetryNodes!==0||desktop.impactText!==false||desktop.editorialText!==true||desktop.availabilityBoard!==true||desktop.availabilityRows<1||desktop.reportedTimelines!==desktop.reportedTimelinesShown||desktop.availabilityText!==true||desktop.readabilityAuthority!=='5'||desktop.columnCount!==5||desktop.columnDisplay!=='grid'||desktop.columnFont<9.5||desktop.teamColumn!==true||desktop.renderedColumns!==5||desktop.orderedColumns!==true||desktop.playerFont<18||desktop.teamFont<11||desktop.injuryFont<12||desktop.statusFont<10||desktop.timelineFont<13||desktop.footFont<9.5||desktop.text<1500)pass=false;
