/* Full-page screenshots of any Player DNA route against the local dev server.
 * node scripts/playerdna-shot.mjs <outDir> [width,...] [tab,...]
 *   PBE_ROUTE=qbdna|wrdna|rbdna|tedna   (default wrdna)
...
const ROUTE = process.env.PBE_ROUTE || 'wrdna';
...
for (const width of WIDTHS) {
  ...
  for (const tab of TABS) {
    ...
    const name = `${ROUTE}-${tab}-${width}.png`;
