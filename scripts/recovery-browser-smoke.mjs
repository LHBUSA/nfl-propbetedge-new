...
if(!nav||typeof nav!=='object'||nav.quickDisplay!=='none'||nav.primary.length!==primaryRoutes.length||nav.research.length!==researchRoutes.length||primaryRoutes.some(r=>!nav.primary.includes(r))||researchRoutes.some(r=>!nav.research.includes(r))||nav.active.length!==1||nav.active[0]!=='home'||Number(nav.primaryWidth)<900)pass=false;
