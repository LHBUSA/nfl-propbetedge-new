...
(() => {
  ...
  function teamLogo(t){return t?.logo?`<img src="${esc(t.logo)}" alt="">`:'<span></span>'}
  ...
  function shellHtml(){const d=dateParts();return `<div id="pbe-sports-shell"><div class="pbes-top"><div class="pbes-brand" data-route="home"><div class="pbes-mark">🏈</div><div><div class="pbes-brand-name"><span style="color:var(--nfl-gold)">NFL</span><span style="color:#44505c">.</span>PropBet<em>Edge</em><span style="color:#44505c;font-size:.7em">.ai</span></div><div class="pbes-brand-sub">Football intelligence · PropBetEdge.ai</div></div></div><div class="pbes-center"><div id="pbes-live-pill" class="pbes-live-pill">Connecting to NFL slate…</div></div><div class="pbes-right"><div class="pbes-date"><strong>${d.day}</strong><span>${d.date} · ET</span></div><button class="pbes-head-btn" type="button" id="pbes-search">⌘ K · Search</button><button class="pbes-head-btn pro" type="button" id="pbes-account">NFL Pro</button></div></div><div class="pbes-news"><div class="pbes-news-badge">PBE NEWS</div><div class="pbes-news-vp"><div class="pbes-news-track" id="pbes-news-track"><div class="pbes-news-empty">Loading current NFL headlines…</div></div></div></div><div class="pbes-scorebar"><div class="pbes-score-label" id="pbes-score-label">NFL</div><div class="pbes-scores" id="pbes-scores"><div class="pbes-news-empty">Loading tonight's games…</div></div></div><div class="pbes-primary"><span class="pbes-nav-label">NFL</span>${PRIMARY.map(([r,l,c])=>`<button type="button" class="pbes-nav-btn ${c}" data-route="${r}">${l}${r==='pbecast'?'<span class="badge" id="pbes-cast-badge">CAST</span>':''}</button>`).join('')}<a class="pbes-nav-btn" style="display:inline-flex;align-items:center;text-decoration:none" href="https://propbetedge.ai/news/nfl">News Site ↗</a></div><div class="pbes-research"><span class="pbes-nav-label">Research</span>${RESEARCH.map(([r,l])=>`<button type="button" class="pbes-nav-btn" data-route="${r}">${l}</button>`).join('')}</div></div>`;}
  ...
  function renderScores(){const games=allGames(),live=liveGames(),label=document.getElementById('pbes-score-label'),host=document.getElementById('pbes-scores'),pill=document.getElementById('pbes-live-pill'),cast=document.querySelector('.pbes-nav-btn.cast'),castBadge=document.getElementById('pbes-cast-badge');if(!host)return;
    ...
    host.innerHTML=ordered.map(g=>{const a=g.teams?.away||{},h=g.teams?.home||{},sem=g.status?.semantics||'UNAVAILABLE';return `<button type="button" class="pbes-score ${sem==='LIVE'?'live':''}" data-cast-game="${esc(g.id)}">${teamLogo(a)}<span class="pbes-score-team">${esc(a.abbreviation||'AWY')}</span><b class="pbes-score-num">${esc(scoreValue(a,sem))}</b><span class="pbes-score-team">${esc(h.abbreviation||'HME')}</span><b class="pbes-score-num">${esc(scoreValue(h,sem))}</b><span class="pbes-score-meta">${esc(sem)} · ${esc(statusText(g))}</span></button>`}).join('');
