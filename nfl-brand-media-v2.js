/* PropBetEdge NFL — brand/media/auth integration v2.1
 * Route-safe media hydration for real NFL team logos and player headshots.
 * No page-wide MutationObserver: bounded render bursts only.
 */
(() => {
  'use strict';
  ...
  function teamLogo(abbr){const raw=String(abbr||'').replace(/[^A-Za-z]/g,'').toUpperCase();const clean=(TEAM_PATH_ALIAS[raw]||raw).toLowerCase();return clean?`https://a.espncdn.com/i/teamlogos/nfl/500/scoreboard/${clean}.png`:''}
  function resolveAbbr(nameOrAbbr){const raw=String(nameOrAbbr||'').trim();if(/^[A-Za-z]{2,4}$/.test(raw))return raw.toUpperCase();const key=normalize(raw);if(TEAM_ABBR.has(key))return TEAM_ABBR.get(key);for(const [name,abbr] of TEAM_ABBR.entries())if(key&&(name.includes(key)||key.includes(name)))return abbr;return''}

  function brandPaywall(){const modal=document.querySelector('.pbe-pro-modal');if(!modal||modal.dataset.pbeBranded==='2')return;modal.dataset.pbeBranded='2';const kicker=modal.querySelector('.pbe-pro-kicker');if(kicker)kicker.innerHTML=`<img class="pbe-paywall-logo" src="${PBE_FULL}" alt="PropBetEdge"><span class="pbe-paywall-label">NFL Pro</span>`}
  function logoImg(abbr,label,eager=false){const img=document.createElement('img');img.className='pbe-official-team-logo';img.dataset.pbeOfficial='1';img.dataset.teamAbbr=abbr;img.src=teamLogo(abbr);img.alt=`${label||abbr} logo`;img.loading=eager?'eager':'lazy';img.decoding='async';img.addEventListener('error',()=>{const fallback=document.createElement('strong');fallback.className='pbe-team-logo-fallback';fallback.textContent=abbr;img.replaceWith(fallback)},{once:true});return img}
  function fillLogo(holder,nameOrAbbr,eager=false){if(!holder)return;const abbr=resolveAbbr(nameOrAbbr);if(!abbr)return;const existing=holder.querySelector('img');if(existing){if(existing.dataset.pbeFallbackBound!=='1'){existing.dataset.pbeFallbackBound='1';existing.loading=eager?'eager':'lazy';existing.decoding='async';existing.addEventListener('error',()=>{if(!existing.isConnected)return;const fallback=logoImg(abbr,nameOrAbbr,eager);if(fallback.src===existing.src){const text=document.createElement('strong');text.className='pbe-team-logo-fallback';text.textContent=abbr;existing.replaceWith(text)}else existing.replaceWith(fallback)},{once:true})}return}holder.replaceChildren(logoImg(abbr,nameOrAbbr,eager))}
  ...
  function repairDashboardSlate(){document.querySelectorAll('.home5-card-row').forEach(row=>{const abbr=row.querySelector('b')?.textContent?.trim()||'';const resolved=resolveAbbr(abbr);if(!resolved)return;const existing=row.querySelector('img');if(existing){if(existing.dataset.pbeFallbackBound==='1')return;existing.dataset.pbeFallbackBound='1';existing.loading='lazy';existing.decoding='async';existing.addEventListener('error',()=>{if(existing.isConnected)existing.replaceWith(logoImg(resolved,abbr,false))},{once:true});return}const placeholder=row.querySelector(':scope > span');if(placeholder)placeholder.replaceWith(logoImg(resolved,abbr,false));else row.prepend(logoImg(resolved,abbr,false))})}
  function repairNamedTeamLogos(){[['.pbe16-team','.pbe16-team-name','.pbe16-crest',true],['.pbe26-card','.pbe26-card-name','.pbe26-crest',false],['.pbe26-brandline','.pbe26-drawer-name','.pbe26-drawer-crest',true]].forEach(([rootSel,nameSel,holderSel,eager])=>{document.querySelectorAll(rootSel).forEach(root=>{const name=root.querySelector(nameSel)?.textContent?.trim()||'';fillLogo(root.querySelector(holderSel),name,eager)})})}
  function repairArchiveCrests(){document.querySelectorAll('.pbe6-podium svg,.pbe6-table svg,.pbe7-standings svg,.pbe11-sb svg').forEach(svg=>{if(svg.dataset.pbeArchiveLogo==='1')return;const abbr=String(svg.querySelector('text')?.textContent||'').trim().toUpperCase();if(!/^[A-Z]{2,4}$/.test(abbr)||['OAK','STL','SD'].includes(abbr))return;svg.dataset.pbeArchiveLogo='1';svg.replaceWith(logoImg(abbr,abbr,false))})}
  ...
  function scan(){brandPaywall();repairScheduleLogos();repairPrimaryGameLogos();repairDashboardSlate();repairNamedTeamLogos();repairArchiveCrests();repairDirectPlayerImages();enhancePlayerMedia()}
  function scheduleScan(){clearTimeout(scanTimer);scanTimer=setTimeout(scan,35)}
  function burstScan(){const token=++burstToken;[0,100,350,900,1800,3500,6500].forEach(delay=>setTimeout(()=>{if(token===burstToken)scan()},delay))}
  function init(){burstScan();['pbe:upgrades-ready','pbe:route-changed','pbe:pro-state','pbe:event-changed'].forEach(name=>window.addEventListener(name,burstScan));document.addEventListener('DOMContentLoaded',burstScan,{once:true})}
