...
OUT = f'data/dist/{POS.lower()}-dna-dataset.json'
MIN_GAMES = 8
...
def main():
    ...
    wr_ids = set(P[P['position'] == POS]['gsis_id'].dropna())
    R = R[R['receiver_player_id'].isin(wr_ids)].copy()
    print(f'{POS}-only receiver-games: {len(R):,}')

    counts = R.groupby('receiver_player_id').size()
    qualified = set(counts[counts >= MIN_GAMES].index)

    active = {}
    apath = f'data/dist/active-{POS.lower()}s-2026.json'
    if os.path.exists(apath):
        with open(apath, encoding='utf-8') as fh:
            aud = json.load(fh)
        for r in aud.get('players') or aud['receivers']:
    ...
    print(f'qualified by history: {len(qualified)}   active 2026 {POS}s: {len(active)}   '
          f'union: {len(include)}')
    ...
    for _, r in sel.iterrows():
        ...
        rows.append({
            ...
            # red zone: derived only from a targeted play with a source-supplied
            # field position inside the opponent twenty
            'rzt': jnum(r.get('rz_targets')), 'rzr': jnum(r.get('rz_receptions')),
            'rztd': jnum(r.get('rz_tds')), 'trz': jnum(r.get('team_rz_targets')),
        })
    ...
    meta = {
        ...
        'product': f'{POS} DNA',
        # The scope sentence is PART OF THE PRODUCT — it is printed in the
        # Sources panel — so it has to name the position this build actually
        # ran for. Hardcoding "WR" here shipped a tight-end dataset that
        # described itself as a receiver one.
        'position_scope': f'{POS} only. Every other position is excluded from the '
                          'product but remains in the team denominators, so target '
                          'share means share of ALL of a team\'s targets.',
        'seasons': seasons,
        ...
        'count_rules': {
            ...
            'red_zone_target': 'a target on a play whose source-supplied yardline is '
                               'inside the opponent twenty. Field position is present on '
                               '100% of targeted plays, measured.',
        },
        'withheld_fields': [
