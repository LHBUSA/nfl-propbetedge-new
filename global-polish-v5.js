...
(() => {
  ...
  function reorderIntelligence(){const group=document.getElementById('intelligence-nav-group');if(!group)return;['nav-home','nav-games','nav-propboard','nav-marketwatch','nav-matchups','nav-picks','nav-qbdna','nav-wrdna','nav-pbepicks','nav-trackrecord','nav-simulator','nav-sgplab','nav-usage','nav-propchain','nav-pbecast','nav-newsintel','nav-injuries','nav-trades'].forEach(id=>{const el=document.getElementById(id);if(el)group.appendChild(el)});}
