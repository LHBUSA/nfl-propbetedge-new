/* PBE Touchdown Targets — the same-origin read contract, on Cloudflare.
 *
 * Ported from api/pbe-touchdown-targets.js (Vercel) with the logic unchanged.
 * What differs is plumbing only: Worker env instead of process.env, service
 * bindings for nfl-current and the engine run ledger, and requirePro asking
 * the one NFL session authority (propbetedge-nfl-auth, internal
 * /internal/v1/session-verdict over the AUTH Service Binding) instead of
 * verifying the cookie itself.
 *
...
const serviceSecret = () => String(ENV.SUPABASE_SERVICE_ROLE_KEY || '').trim();
/* A Worker cannot fetch another Worker's workers.dev URL in the same account
 * (error 1042), so nfl-current and the engine run ledger are reached through
 * Service Bindings; the URL host is ignored by a binding. */
const viaBinding = binding => (binding && typeof binding.fetch === 'function' ? (url, init) => binding.fetch(new Request(url, init)) : undefined);
const seasonOpts = () => ({ gatewayUrl: 'https://nfl-current.internal', fetchImpl: viaBinding(ENV.NFL_CURRENT) });
const engineOpts = () => ({ engineUrl: 'https://nfl-game-picks-orchestrator.internal', fetchImpl: viaBinding(ENV.PICKS_ENGINE) });
...
async function governance(secret) {
  const [models, observations, runtime, current, latestAudit] = await Promise.all([
    ...
    engineRuntime(TD_LANES, engineOpts()),
    currentSeason(seasonOpts()).catch(() => null),
...
/* The session answer comes from the one NFL session authority. Only the
 * session cookie values are forwarded; the verdict carries no identity. A
 * verdict that cannot be obtained is treated like getNflSession's degraded
 * answer: 503, never a grant. */
async function sessionVerdict(req) {
  const cookie = req.headers.get('cookie') || '';
  const values = name => cookie.split(';').map(part => part.trim()).filter(part => part.startsWith(`${name}=`))
    .map(part => part.slice(name.length + 1)).filter(Boolean);
  const tokens = [...values('pbe_nfl_session_v2'), ...values('pbe_nfl_session')].slice(0, 4);
  if (!tokens.length) return { valid: false, pro: false, signed_in: false, access: 'anonymous', degraded: false, stage: 'no_cookie' };
  const token = String(ENV.NFL_AUTH_INTERNAL_TOKEN || '').trim();
  if (!ENV.AUTH || typeof ENV.AUTH.fetch !== 'function' || token.length < 32) {
    return { valid: false, pro: false, signed_in: false, access: 'unavailable', degraded: true, stage: 'session_authority_unconfigured' };
  }
  try {
    const response = await ENV.AUTH.fetch(new Request('https://propbetedge-nfl-auth.internal/internal/v1/session-verdict', {
      method: 'POST',
      headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
      body: JSON.stringify({ tokens }),
    }));
    if (!response.ok) throw new Error(`session_authority_${response.status}`);
    const verdict = await response.json();
    if (typeof verdict?.access !== 'string') throw new Error('session_authority_shape');
    return verdict;
  } catch (error) {
    return { valid: false, pro: false, signed_in: false, access: 'unavailable', degraded: true, stage: 'session_authority_unavailable' };
  }
}

async function requirePro(req, res) {
  const auth = await sessionVerdict(req);
  const email = auth.signed_in === true;
  if (!email) {
...
/* ---- Worker plumbing: a Node-style res shim, so the contract above keeps its
 * exact send(res, status, body, cacheControl) calls. ------------------------- */
export const SERVICE = 'nfl-touchdown-targets-api';
...
export async function handle(request, env) {
  ...
  const req = { method: request.method, query: Object.fromEntries(url.searchParams), headers: request.headers };
  ...
  headers.set('x-pbe-served-by', `${SERVICE}/${VERSION}`);
  return new Response(request.method === 'HEAD' ? null : res.body, { status: res.statusCode, headers });
}
  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/health') {
      return Response.json({
        ...
        requirements: {
          SUPABASE_SERVICE_ROLE_KEY: Boolean(String(env.SUPABASE_SERVICE_ROLE_KEY || '').trim()),
          NFL_AUTH_INTERNAL_TOKEN: String(env.NFL_AUTH_INTERNAL_TOKEN || '').trim().length >= 32,
          AUTH_BINDING: Boolean(env.AUTH), NFL_CURRENT_BINDING: Boolean(env.NFL_CURRENT), PICKS_ENGINE_BINDING: Boolean(env.PICKS_ENGINE),
        },
      }, { headers: { 'cache-control': 'no-store' } });
    }
    if (url.pathname !== '/api/pbe-touchdown-targets') {
      return Response.json({ error: 'not_found', service: SERVICE }, { status: 404, headers: { 'cache-control': 'no-store' } });
    }
