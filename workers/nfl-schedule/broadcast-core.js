...
export function observeEspnEvent(event, source) {
  ...
  return {
    ...
    venue: venueFromEspn(comp?.venue),
    neutral_site: typeof comp?.neutralSite === 'boolean' ? comp.neutralSite : null,
    source
  };
}

/* The venue ESPN lists for this competition: the actual stadium, neutral
   sites included. `indoor` is ESPN's own flag; absent means unknown, not false. */
export function venueFromEspn(v) {
  if (!v || typeof v !== 'object' || !S(v.fullName || v.name)) return null;
  const a = v.address && typeof v.address === 'object' ? v.address : {};
  return {
    name: S(v.fullName || v.name),
    city: S(a.city ?? v.city) || null,
    state: S(a.state ?? v.state) || null,
    country: S(a.country ?? v.country) || null,
    indoor: typeof v.indoor === 'boolean' ? v.indoor : null,
    espn_venue_id: /^\d+$/.test(S(v.id)) ? S(v.id) : null
  };
}
...
export function observeRelayGame(game, source) {
  ...
  return {
    ...
    venue: venueFromEspn(game?.venue),
    neutral_site: typeof game?.neutral_site === 'boolean' ? game.neutral_site : null,
    source
  };
}
...
export function mergeObservations(snapshot, observations, fetchedAt) {
  ...
  for (const o of A(observations)) {
    ...
    /* The relay carries a venue name but not ESPN's indoor or neutral-site
       flags: a same-venue relay read keeps what the CDN observed. */
    const sameVenue = Boolean(prev?.venue && o.venue && prev.venue.name === o.venue.name);
    const venue = o.venue ? { ...o.venue, indoor: o.venue.indoor ?? (sameVenue ? prev.venue.indoor : null) } : (prev?.venue || null);
    const neutralSite = o.neutral_site ?? (sameVenue ? prev.neutral_site ?? null : null);
    const next = {
      event_id: o.event_id, week: o.week, kickoff: o.kickoff, time_valid: timeValid,
      away: o.away, home: o.home, channels, venue, neutral_site: neutralSite, source: o.source,
