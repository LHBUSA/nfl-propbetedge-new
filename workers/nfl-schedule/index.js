 ...
 *                   + venue (ESPN's per-game stadium + roof), + kickoff
...
import { buildVenue, buildKickoff } from './game-facts.js';
...
export default {
  async fetch(request, env) {
    ...
    const enriched = games.map(g => {
      let broadcast, venue = null, kickoff = null;
      ...
      try { venue = buildVenue(g, snap); kickoff = buildKickoff(g, snap); }
      catch (e) { venue = { status: 'UNAVAILABLE', reason: `venue_build_failed:${String(e?.message || e).slice(0, 60)}` }; }
      return { ...g, broadcast, venue, kickoff };
    });
