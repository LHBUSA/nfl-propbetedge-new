/* PropBetEdge NFL — global OS polish v4 */
(() => {
  'use strict';
  ...
  function reorderIntelligence() {
    ...
    const desired=['nav-home','nav-games','nav-propboard','nav-marketwatch','nav-matchups','nav-picks','nav-simulator','nav-sgplab','nav-usage','nav-propchain','nav-pbecast','nav-injuries','nav-trades'];
  ...
  function renameGroups() {
    ...
    const status=document.querySelector('.sf-status');if(status)status.textContent='Games · market · model · SGP · usage · news · archive';
    const api=document.querySelector('.sf-api');if(api)api.innerHTML='NFL Pro · $9.99/week <span class="pbe-os-shortcut"><kbd>Ctrl K</kbd> search</span> <span class="pbe-os-shortcut"><kbd>Shift E</kbd> event</span>';
  }

  function rebuildQuickNav() {
    const nav=document.querySelector('.pbe-v2-quicknav');if(!nav)return;
    nav.innerHTML=`<button onclick="App.nav('home')">Dashboard</button><button onclick="App.nav('games')">Games</button><button class="primary" onclick="App.nav('propboard')">Props</button><button onclick="App.nav('marketwatch')">Watch</button><button onclick="App.nav('matchups')">Matchups</button><button onclick="App.nav('picks')">Model</button><button onclick="App.nav('simulator')">Sim</button><button onclick="App.nav('sgplab')">SGP</button><button onclick="App.nav('pbecast')">Game</button>`;
  }
  ...
  function wrapNav() {
    if(!window.App?.nav||App.__pbeOsPolishV4Wrapped)return;
    const previous=App.nav.bind(App);
    App.nav=function(route){const result=previous(route);setCommandMeta(route);setTimeout(()=>setCommandMeta(route),0);setTimeout(()=>setCommandMeta(route),120);return result;};
    App.__pbeOsPolishV4Wrapped=true;
  }
