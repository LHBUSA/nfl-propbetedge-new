/* GET /api/te-dna/compare
 *
 *  mode=players  ?player_a=00-0033857&player_b=00-0034785
 *      Two tight ends side by side on their own baselines, plus how each one
 ...
 *      This game's conditions placed against that tight end's own history.
 *
  ...
  for (const key of Object.keys(CONDITIONS)) {
    ...
    if (!na || !nb) {
      conditions[key] = { available: false, ...meta, games_a: na, games_b: nb,
        reason: !na && !nb ? 'neither tight end has a game in this window'
              : !na ? 'tight end A has no game in this window'
              : 'tight end B has no game in this window' };
      continue;
    }
  ...
  for (const key of on) {
    ...
    if (!n) {
      windows[key] = { available: false, label: CONDITIONS[key].label, games: 0,
        reason: 'this tight end has no game in this window', coverage: s.coverage || undefined };
      continue;
    }
