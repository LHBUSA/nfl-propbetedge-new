/* GET /api/te-dna
 *   ?player_id=00-0033857            GSIS id (preferred)
 *   ?espn_id=15847                 ESPN athlete id
 *   ?name=Travis%20Kelce         exact name only, never fuzzy
 *   ?list=1                          the tight ends this dataset can answer for
 *
 * Baseline, RED ZONE DNA, recency, condition matrix, DNA signals, form series
 * and the QB connection. It shares the receiver warehouse and the receiver
 * arithmetic with WR DNA, but it leads with the red zone, because that is
 * where a tight end's value concentrates and where his touchdown market lives.
 */
import { resolvePlayer, gamesFor, baseline, conditionProfile, dnaSignals,
         qbConnections, provenance, dataWindow, SAMPLE, dataset }
  from './_tedna/engine.js';
...
/** Red-zone block for one window, carrying its own N and never a bare rate. */
function redZone(rows, label) {
  const b = baseline(rows);
  if (!b) return { label, available: false, games: 0, reason: 'no games in this window' };
  if (!b.rz_targets) {
    return { label, available: true, games: b.games, rz_targets: 0,
             rz_games: 0, has_red_zone_work: false,
             note: 'no red-zone target in this window' };
  }
  return {
    label, available: true, games: b.games, has_red_zone_work: true,
    rz_targets: b.rz_targets, rz_receptions: b.rz_receptions,
    rz_touchdowns: b.rz_touchdowns,
    rz_games: b.rz_games,
    rz_target_share: b.rz_target_share,
    rz_catch_rate: b.rz_catch_rate,
    rz_td_per_target: b.rz_td_per_target,
    rz_targets_per_game: +(b.rz_targets / b.games).toFixed(2),
    share_of_own_targets: b.targets ? {
      numerator: b.rz_targets, denominator: b.targets,
      pct: +(100 * b.rz_targets / b.targets).toFixed(1) } : null,
    sample_label: SAMPLE(b.rz_targets),
    sample_basis: 'red-zone TARGETS, not games'
  };
}
...
export default function handler(req, res) {
  ...
  if (q.list) {
    ...
    const rows = D.players.map(p => {
      ...
      return {
        ...
        name: p.display_name, position: p.position ?? 'TE',
  ...
  /* A tight end on a 2026 roster with no NFL history is a real, expected state.
  ...
  if (!rows.length) {
    return send(res, 200, {
      ...
      reason: p.active_2026
        ? 'This tight end is on a 2026 roster but has no completed NFL game inside our data window.'
        : 'No completed NFL game for this player inside our data window.',
      player: identity, nfl_games: 0,
      baseline: null, red_zone_dna: null, recent: null, conditions: null,
      dna_signals: null, qb_connection: null, game_log: [],
  ...
  send(res, 200, {
    ...
    /* RED ZONE DNA — the dimension the tight-end product leads with.
       A red-zone target is a target whose source-supplied yardline is inside
       the opponent twenty. Every rate keeps its numerator and denominator, and
       a back-of-the-endzone conversion rate on three targets is labelled a very
       small sample rather than presented as a finishing skill. */
    red_zone_dna: {
      career: redZone(rows, 'Career in window'),
      current_season: { ...redZone(seasonRows, `${latest} season`), season: latest },
      last_10: redZone(rows.slice(-10), 'Last 10'),
      definition: 'A target inside the opponent 20-yard line, taken from the '
                + 'source-supplied yardline. Yardline coverage on targets is 100%.',
      caveat: 'Red-zone volume is small by nature. A conversion rate on a handful '
            + 'of targets describes those targets and nothing more.'
    },
    recent: {
    ...
    /* Shared with WR DNA. A tight end's passer relationship matters as much. */
