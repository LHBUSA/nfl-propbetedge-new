/* GET /api/wr-dna/prop-lab
 *   ?player_id=00-0036322&event_id=<market event id>[&condition=dome]
 *
...
import { resolvePlayer, gamesFor, propThreshold, tdHistory, splitRows,
         provenance, dataWindow, MARKETS, CONDITIONS } from '../_wrdna/engine.js';
...
export default async function handler(req, res) {
  ...
  const identity = {
    ...
    position: p.position ?? 'WR', matched_by: found.matched_by,
  ...
  if (!rows.length) {
    return send(res, 200, {
      ...
      reason: 'no completed NFL game for this receiver inside our data window',
  ...
  let board;
  try {
    board = await eventMarkets(String(q.event_id), 'receiving');
  } catch (e) {
    ...
    if (!offered) {
      return { market, market_label: label, available: false, state: MARKET_UNAVAILABLE,
        reason: board.available
          ? 'no book in the current market is offering this market for this receiver'
