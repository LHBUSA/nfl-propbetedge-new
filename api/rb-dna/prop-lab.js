/* GET /api/rb-dna/prop-lab
 *   ?player_id=00-0034844&event_id=<market event id>[&condition=dome]
 *
 * Every currently offered rushing and receiving market for this game, each
 * against its own real line over career / season / last 10 / last 5 / similar
 * conditions.
 *
 ...
 * a count of games with a rushing OR receiving touchdown, and the market PRICE
 * shown as a price. The historical rate is never called an implied probability.
 */
...
function tdWindow(rows, label) {
  ...
  return {
    ...
    rush_tds: t.rush_tds, receiving_tds: t.receiving_tds,
    statement: t.statement, sample_label: t.sample_label
  };
}

/** The per-row value each market is graded on, including the derived ones. */
const VALUE_OF = {
  rushing_yards: r => r.ry, rush_attempts: r => r.car,
  receiving_yards: r => r.recy, receptions: r => r.rec,
  scrimmage_yards: r => (r.ry ?? 0) + (r.recy ?? 0)
};
...
export default async function handler(req, res) {
  ...
  if (!rows.length) {
    return send(res, 200, {
      ...
      reason: 'no completed NFL game for this back inside our data window',
  ...
  let board;
  try {
    board = await eventMarkets(String(q.event_id), 'rushing');
  } catch (e) {
  ...
  const cards = Object.values(RUSHING_MARKET_MAP).map(market => {
    ...
    if (!offered) {
      return { market, market_label: label, available: false, state: MARKET_UNAVAILABLE,
        reason: board.available
          ? 'no book in the current market is offering this market for this back'
    ...
    if (isTd) {
      /* Anytime TD carries a PRICE, not a line. It is represented as a count of
         touchdown games — rushing or receiving, because either one settles the
         market — and the price is labelled as a price. */
      return {
        ...
        distribution: rows.map(r => ({
          ...
          value: (r.rtd ?? 0) + (r.rectd ?? 0),
          outcome: ((r.rtd ?? 0) + (r.rectd ?? 0)) > 0 ? 'TD' : 'NO TD' })),
        note: 'A count of games with a rushing or receiving touchdown. Not an implied '
    ...
    return {
      ...
      distribution: rows.filter(r => typeof valueOf(r) === 'number').map(r => ({
        date: r.d, season: r.s, week: r.w, opponent: r.opp, home: r.ha === 1,
        value: valueOf(r),
        outcome: valueOf(r) > line ? 'OVER' : valueOf(r) < line ? 'UNDER' : 'PUSH',
