/* WR DNA engine — deterministic. Sibling of the QB engine, not a copy of it.
 * =============================================================================
...
export function dataset() {
  if (DATA) return DATA;
  const p = join(process.cwd(), 'data', 'dist', 'wr-dna-dataset.json');
  DATA = JSON.parse(readFileSync(p, 'utf8'));
  DATA._byPlayer = new Map();
  for (const g of DATA.receiver_games) {
...
export const TD_MARKET = { key: 'rtd', label: 'Anytime TD', gateway: 'player_anytime_td' };
 ...
 */
export function qbConnections(gsis, opts = {}) {
...
export function tdHistory(rows) {
