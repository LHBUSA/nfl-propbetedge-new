...
(() => {
  ...
  /* scope is a list of entities the story is about, but it rendered as a bare
     name directly under the headline and read as a byline -- so an unrelated
     tag looked like an attribution. It is now labelled, and only entities the
     article's own text corroborates are shown (see pbe-news-trust.js). */
  function impact(item){const m=item?.market_impact||{};const scope=window.PBENewsTrust?window.PBENewsTrust.safeScope(item):(m.scope||'');return{band:String(m.band||'CONTEXT').toUpperCase(),text:m.text||'Contextual NFL information. No verified sportsbook price movement is being claimed.',scope,score:Number.isFinite(Number(m.score))?Number(m.score):null}}
