/* PropBetEdge NFL — global OS polish v3 */
(() => {
  'use strict';
  ...
  function reorderIntelligence() {
    ...
    const desired=['nav-home','nav-propboard','nav-marketwatch','nav-matchups','nav-picks','nav-simulator','nav-sgplab','nav-usage','nav-propchain','nav-pbecast','nav-injuries','nav-trades'];
  ...
  function renameGroups() {
    ...
    const status=document.querySelector('.sf-status');if(status)status.textContent='Market · model · SGP · usage · news · archive';
  ...
  function rebuildQuickNav() {
    ...
    nav.innerHTML=`<button onclick="App.nav('home')">Dashboard</button><button class="primary" onclick="App.nav('propboard')">Props</button><button onclick="App.nav('marketwatch')">Watch</button><button onclick="App.nav('matchups')">Matchups</button><button onclick="App.nav('picks')">Model</button><button onclick="App.nav('simulator')">Sim</button><button onclick="App.nav('sgplab')">SGP</button><button onclick="App.nav('pbecast')">Game</button><a href="https://propbetedge.ai/news/nfl">News ↗</a>`;
  }
  ...
  function wrapNav() {
    ...
    if(App.__pbeOsPolishV3Wrapped)return;
    ...
    App.__pbeOsPolishV3Wrapped=true;
  }
